...
- **Unstructured / Semantic Retrieval**: Pensieve requires vector embeddings and similarity search over wizarding world lore articles, news digests, and canonical source texts to power its AI companion and RAG (Retrieval-Augmented Generation) pipeline.
...
2. **Pensieve Vector Retrieval Baseline**:
   - Pensieve will utilize the **`pgvector`** extension in PostgreSQL for storing embeddings and executing nearest-neighbor similarity queries (using HNSW or IVFFlat index structures).
...
- **Transactional Consistency**: Embeddings are updated in the same ACID transactions as document content and metadata, completely preventing orphaned vectors or index desynchronization.
...
### Trade-offs
- While `pgvector` scales effectively to millions of vectors, ultra-high-throughput vector search across hundreds of millions of embeddings may eventually warrant a dedicated vector search engine at higher product maturity.
...
1. **MongoDB / Document Databases**:
   - *Rationale for Rejection*: Document stores lack the rigid relational integrity, foreign keys, and transactional guarantees essential for financial-style house point ledgers, strict friend consent graphs, and quiz scoring.
2. **Standalone Vector Database (e.g., Pinecone, Qdrant, Weaviate) from Day 1**:
   - *Rationale for Rejection*: Adds an extra distributed service, separate authentication, and dual-write synchronization overhead before data volume justifies the operational burden.
3. **Firebase / Supabase as Architecture**:
   - *Rationale for Rejection*: Supabase and Firebase are hosting/platform providers rather than portable database technologies. Our architecture specifies standard PostgreSQL so it remains fully portable across hosting environments (including Supabase, managed cloud PostgreSQL, or containerized instances).
4. **Multiple Specialized Databases from Day 1**:
   - *Rationale for Rejection*: Prematurely introducing separate relational, key-value, document, and vector databases multiplies infrastructure complexity without architectural necessity.
